...
class GP_Lofi_Standalone_Route {
    ...
    return [
        'svg' => [
            ...
            'class' => true, 'style' => true, 'aria-hidden' => true, 'role' => true,
    ...
    public function render_lofi_if_matched() {
        ...
        if (!file_exists($html_file)) { status_header(500); wp_die('Thiếu templates/lofi.html'); }
        $html = file_get_contents($html_file);
        // Chèn file JS tách riêng trước </body>
        $injection = "\n<script src=\"" . esc_url($js_url) . "\"></script>\n";
        if (stripos($html, '</body>') !== false) {
            $html = preg_replace('/<\/body>\s*<\/html>\s*$/i', $injection . '</body></html>', $html);
        } else {
            $html .= $injection;
        }
        ...
        <div class="wrap">
            ...
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                ...
                <table class="widefat striped" id="gp-table-playlist">
                    <thead><tr><th>Tiêu đề</th><th>Nghệ sĩ</th><th>Audio URL</th><th>Cover URL</th><th style="width:60px">Xóa</th></tr></thead>
                    <tbody>
                ...
                <table class="widefat striped" id="gp-table-nature">
                    <thead><tr><th>ID</th><th>Tên hiển thị</th><th>Audio URL</th><th>Icon SVG (inline)</th><th style="width:60px">Xóa</th></tr></thead>
                    <tbody>
    ...
    public function handle_save() {
        ...
        }, $playlist), function($r){ return !empty($r['src']) || !empty($r['title']); }));
...
}, $nature), function($r){ return !empty($r['id']); }));
...
}, $instrument), function($r){ return !empty($r['id']); }));
